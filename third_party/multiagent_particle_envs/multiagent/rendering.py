...
class Viewer(object):
    ...
    def render(self, return_rgb_array=False):
        ...
        if return_rgb_array:
            ...
            # --- UNIVERSAL FIX (Compatible with Pyglet 2.x and NumPy 2.x) ---
            import numpy as np

            # 1. Get the raw bytes (Handles both old and new Pyglet)
            if hasattr(image_data, "get_data"):
                # Pyglet 2.0+ uses get_data() and requires a format string
                raw_data = image_data.get_data("RGB", image_data.width * 3)
            else:
                # Pyglet 1.x uses .data
                raw_data = image_data.data

            # 2. Convert to NumPy array (Handles NumPy 2.0 removing fromstring binary support)
